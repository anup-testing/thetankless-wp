...
use DateTime;
use Exception;
use WPDRMS\ASL\Core\Models\SearchInstance;
use WPDRMS\PluginCore\Traits\SingletonTrait;
use WPDRMS\ASL\Statistics\ORM\Interaction;
use WPDRMS\ASL\Statistics\ORM\Result;
...
class SearchQuery {
	...
	public function getSearchesVolume( array $args = array() ): array {
		...
		$args['cached']            = $args['cached'] === null ? $args['cached'] : ( $args['cached'] ? 1 : 0 );
		$args['suggested']         = $args['suggested'] === null ? $args['suggested'] : ( $args['suggested'] ? 1 : 0 );
		...
		if ( $args['compare'] ) {
			...
			$prev_sql = $wpdb->prepare(
                ...
                0                                                 AS results_shown_prev,
                0                                                 AS interactions_prev
             FROM $search_table s
	...
	public function getSearches( array $args = array() ): array {
		...
		if ( $variation === 'popular' ) {
			...
			if ( empty($count_params) ) {
				$count_sql = "SELECT COUNT(DISTINCT s.{$args['popular_field']}) FROM $search_table s WHERE $where_sql $interaction_sub";
			} else {
			...
			if ( empty($count_params) ) {
				$count_sql = "SELECT COUNT(*) FROM $search_table s WHERE $where_sql $interaction_sub";
			} else {
				$count_sql = $wpdb->prepare(
		...
		if ( $variation === 'popular' ) {
            ...
            0                                                   AS average_results_per_search,
            0                                                   AS average_interaction_per_search,
         ...
         LIMIT %d OFFSET %d",
			...
			$compare_data = null;
			if ( $args['compare'] && ! empty( $results ) && isset( $start_date, $end_date ) ) {
				$period_seconds  = strtotime( $end_date ) - strtotime( $start_date );
				$prev_end_date   = date( 'Y-m-d 23:59:59', strtotime( $start_date ) - 1 );
				$prev_start_date = date( 'Y-m-d 00:00:00', strtotime( $prev_end_date ) - $period_seconds );
                ...
                0        AS average_results_per_search_prev,

                0        AS average_interaction_per_search_prev
