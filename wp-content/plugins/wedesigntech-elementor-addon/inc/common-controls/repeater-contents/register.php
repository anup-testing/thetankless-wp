...
class WeDesignTech_Common_Controls_Repeater_Contents {
		...
		$elementor_object->start_controls_section( 'wdt_section_content', array(
			...
			if(in_array('default', $this->options_group)) {
				...
				if(in_array('social_icons', $this->option_default_keys)) {
					$repeater->add_control( 'instagram_link', array(
						'label'       => esc_html__( 'Instagram Link', 'wdt-elementor-addon' ),
						'type'        => \Elementor\Controls_Manager::URL,
						'label_block' => true,
						'placeholder' => esc_html__( '#', 'wdt-elementor-addon' ),
						'condition'   => $this->option_default_condition
					) );
					$repeater->add_control( 'meta_link', array(
						'label'       => esc_html__( 'Meta Link', 'wdt-elementor-addon' ),
						'type'        => \Elementor\Controls_Manager::URL,
						'label_block' => true,
						'placeholder' => esc_html__( '#', 'wdt-elementor-addon' ),
						'condition'   => $this->option_default_condition
					) );
					$repeater->add_control( 'thread_link', array(
						'label'       => esc_html__( 'Thread Link', 'wdt-elementor-addon' ),
						'type'        => \Elementor\Controls_Manager::URL,
						'label_block' => true,
						'placeholder' => esc_html__( '#', 'wdt-elementor-addon' ),
						'condition'   => $this->option_default_condition
					...
					$repeater->add_control( 'discard_link', array(
						'label'       => esc_html__( 'Discard Link', 'wdt-elementor-addon' ),
						'type'        => \Elementor\Controls_Manager::URL,
						'label_block' => true,
						'placeholder' => esc_html__( '#', 'wdt-elementor-addon' ),
						'condition'   => $this->option_default_condition
    ...
    public function render_button($item_button_text, $link_start, $link_end) {
		$output = '';
		$characters = str_split($item_button_text);
		$button_text = '';
		$index = 1;
		foreach ($characters as $character) {
			$button_text .= '<span class="wdt-button-char" style="--char-index:'.$index.';">' . htmlentities($character, ENT_QUOTES, 'UTF-8') . '</span>';
			$index++;
		}
		if (!empty($item_button_text) && !empty($link_start)) {
			$output .= '<div class="wdt-content-button wdt-button-clone">';
			$output .= $link_start;
			$output .= '<div class="wdt-button-text">';
			$output .= $button_text;
			$output .= '</div>';
			$output .= $link_end;
			$output .= '</div>';
	...
	public function render_social_icons($item) {
		$output = '';
		if($item['instagram_link']['url'] != '' || $item['meta_link']['url'] != '' || $item['thread_link']['url'] != '' || $item['twitter_link']['url'] != '' || $item['discard_link']['url'] != '') {
			$output .= '<div class="wdt-social-icons-container">';
				$output .= '<ul class="wdt-social-icons-list">';
					if($item['instagram_link']['url'] != '') {

						$target = ( $item['instagram_link']['is_external'] == 'on' ) ? ' target="_blank" ' : '';
						$nofollow = ( $item['instagram_link']['nofollow'] == 'on' ) ? 'rel="nofollow" ' : '';
						$link_start = '<a href="'.esc_url( $item['instagram_link']['url'] ).'"'. $target . $nofollow.'>';
						$link_end = '</a>';

						$output .= '<li>';
							$output .= $link_start;
								$output .= '<i class="icon-social-instagram"></i>';
							$output .= $link_end;
						$output .= '</li>';

					}
					if($item['meta_link']['url'] != '') {

						$target = ( $item['meta_link']['is_external'] == 'on' ) ? ' target="_blank" ' : '';
						$nofollow = ( $item['meta_link']['nofollow'] == 'on' ) ? 'rel="nofollow" ' : '';
						$link_start = '<a href="'.esc_url( $item['meta_link']['url'] ).'"'. $target . $nofollow.'>';
						$link_end = '</a>';

						$output .= '<li>';
							$output .= $link_start;
								$output .= '<i class="icon-social-meta"></i>';
							$output .= $link_end;
						$output .= '</li>';

					}
					if($item['thread_link']['url'] != '') {

						$target = ( $item['thread_link']['is_external'] == 'on' ) ? ' target="_blank" ' : '';
						$nofollow = ( $item['thread_link']['nofollow'] == 'on' ) ? 'rel="nofollow" ' : '';
						$link_start = '<a href="'.esc_url( $item['thread_link']['url'] ).'"'. $target . $nofollow.'>';
						$link_end = '</a>';

						$output .= '<li>';
							$output .= $link_start;
								$output .= '<i class="icon-social-thread"></i>';
							$output .= $link_end;
						$output .= '</li>';
					...
					if($item['twitter_link']['url'] != '') {
						...
						$output .= '<li>';
							$output .= $link_start;
								$output .= '<i class="icon-social-twitter-x"></i>';
							$output .= $link_end;
						$output .= '</li>';

					}
					if($item['discard_link']['url'] != '') {

						$target = ( $item['discard_link']['is_external'] == 'on' ) ? ' target="_blank" ' : '';
						$nofollow = ( $item['discard_link']['nofollow'] == 'on' ) ? 'rel="nofollow" ' : '';
						$link_start = '<a href="'.esc_url( $item['discard_link']['url'] ).'"'. $target . $nofollow.'>';
						$link_end = '</a>';

						$output .= '<li>';
							$output .= $link_start;
								$output .= '<i class="icon-social-discard"></i>';
							$output .= $link_end;
						$output .= '</li>';
