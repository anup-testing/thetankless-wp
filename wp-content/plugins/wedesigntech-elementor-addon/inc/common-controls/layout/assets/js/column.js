(function ($) {
	...
	const wdtColumnWidgetHandler = function($scope, $) {
        ...
        const $scopeColumnsettings = $scope.find('.wdt-column-wrapper');
        if( $scopeColumnsettings.data('column-settings') ) {
            $scope.find('.wdt-column-wrapper').each(function() {
                const $scopeColumn = $(this);
                ...
                if ($scopeColumn.data('column-settings')) {
                    ...
                    if ($get_user_devices !== undefined) {
                        if ($get_user_devices.indexOf($deviceMode) > -1) {
                            ...
                            $scopeColumn.find('.wdt-column-pagination.wdt-snap-scroll-pagination').css({ 'display': 'block' });
                        } else {
                            $scopeColumn.removeClass('wdt-snap-scroll');
                            $scopeColumn.find('.wdt-column-pagination.wdt-snap-scroll-pagination').css({ 'display': 'none' });
                        }
                    }
                }
            });
