		...
		$output .= '<div class="wdt-button-holder '.esc_attr(implode(' ', $classes)).'" id="wdt-button-'.esc_attr($module_id).'">';
			$output .= '<a class="wdt-button" '.$widget_object->get_render_attribute_string('button').'>';
				...
				$output .= '<div class="wdt-button-text">';
					foreach (str_split($settings['text']) as $index => $char) {
						if (trim($char) === '') {
							$output .= '<span style="--char-index:' . esc_attr($index) . ';"></span>'; // Empty span for spaces
						} else {
							$output .= '<span style="--char-index:' . esc_attr($index) . ';">' . esc_html($char) . '</span>';
						}
					}
					if($settings['subtext'] != '') {
