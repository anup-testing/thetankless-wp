	...
	public function render_form_html($settings) {
		...
		$output .= '<form class="wdt-mailchimp-subscribe-form '.esc_attr($button_class).'" name="mailchimpSubscribeForm" action="#" method="post">';
			...
			if($settings['terms_condition'] == 'yes') {
				...
				$output .= '<input type="checkbox" class="wdt-terms-and-conditions" required="required"><span class="wdt-terms-condition-lbl">' . esc_html__(' I agree With the ', 'wdt-elementor-addon') . '</span> <a href="'.esc_attr($privacy_policy_page).'"> <span class="wdt-privacy-policy-lbl">' . esc_html__(' Privacy Policy ', 'wdt-elementor-addon') . '</span> </a>';
				$output .= '</div>';
			}
