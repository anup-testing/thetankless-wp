						...
						foreach( $settings['item_contents'] as $key => $item ) {
							$output .= '<li id="wdt-interactive-showcase-'.esc_attr($key).'">';
								...
								if( $item['item_type'] == 'template' ) {
									...
									$output .= '<div class="wdt-interactive-showcase-content-group">';
										if( !empty($item['item_title']) ) {
											$target = ( isset($item['item_link']['is_external']) && $item['item_link']['is_external'] == 'on' ) ? ' target="_blank" ' : '';
											$nofollow = ( isset($item['item_link']['nofollow']) && $item['item_link']['nofollow'] == 'on' ) ? 'rel="nofollow" ' : '';
											$output .= '<div class="wdt-content-title"><a href="' . esc_url($item['item_link']['url']) . '"' . $target . $nofollow . '>' . esc_html($item['item_title']) . '</a></div>';
											// $output .= '<div class="wdt-content-title">'.esc_html($item['item_title']).'</div>';
										}
										if($settings['icon_show'] == 'true') {
