...
class WeDesignTech_Widget_Base_Heading {
	...
	public function init_scripts() {
		...
		return array (
			...
			$this->name() . '_gsap' => WEDESIGNTECH_ELEMENTOR_ADDON_DIR_URL.'assets/js/gsap.min.js',

			$this->name() . '_gsap_scrolltrigger' => WEDESIGNTECH_ELEMENTOR_ADDON_DIR_URL.'assets/js/ScrollTrigger.min.js'
	...
	public function render_title_html($settings) {
		...
		if (isset($settings['title']) && !empty($settings['title'])) {
			// Check if 'title_vertical_align' exists in the $settings array
			$title_align = isset($settings['title_vertical_align']) ? esc_attr($settings['title_vertical_align']) : 'default-align'; // Use a default value if not set
		
			$output .= '<' . esc_attr($settings['title_tag']) . ' class="wdt-heading-title-wrapper wdt-heading-align-' . $title_align . ' ' . esc_attr($deco_class) . '">';
	...
	public function render_title_splitup_html($title, $colored_elements, $split_heading) {
		...
		foreach ($splitted_titles as $key => $splitted_title) {
			$updated_key = $key + 1;
			$splitted_title = htmlspecialchars($splitted_title, ENT_QUOTES, 'UTF-8'); // Encode special characters minimally
	
			if (in_array($updated_key, $colored_elements_splitted)) {
				if ($split_heading == 'true') {
					$text .= '<span class="wdt-split-heading-wrapper">';
					$splitting_titles = mb_str_split($splitted_title); // Use mb_str_split for multibyte characters
					foreach ($splitting_titles as $splitted_titles_values) {
						$text .= '<span class="wdt-split-heading-title">' . htmlspecialchars($splitted_titles_values, ENT_QUOTES, 'UTF-8') . '</span>'; // Encode special characters minimally
					}
					$text .= '</span> ';
				} else {
	...
	public function render_subtitle_html($settings) {
		...
		if (isset($settings['subtitle']) && !empty($settings['subtitle'])) {
			// Check if 'subtitle_vertical_align' exists in the $settings array
			$subtitle_align = isset($settings['subtitle_vertical_align']) ? esc_attr($settings['subtitle_vertical_align']) : 'default-align'; // Use a default value if not set
		
			$output .= '<div class="wdt-heading-subtitle-wrapper wdt-heading-align-' . $subtitle_align . ' ' . esc_attr($deco_class) . '">';
			...
			$output .= esc_html($settings['subtitle']); // Ensure the subtitle is escaped properly
