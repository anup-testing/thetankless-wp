...
if( !class_exists( 'GlidexPlusBCTemplate' ) ) {
    class GlidexPlusBCTemplate {
        ...
        function load_frontend() {
            add_filter( 'glidex_breadcrumb_params', array( $this, 'register_breadcrumb_params' ) );
            add_filter( 'glidex_breadcrumb_get_template_part', array( $this, 'register_template' ), 10, 2 );

            add_action( 'glidex_after_main_css', array( $this, 'enqueue_assets' ) );

            add_filter( 'glidex_header_wrapper_classes', array( $this, 'register_header_class' ), 20, 1 );
        }

        function register_header_class($classes) {

            $breadcrumb_position = glidex_customizer_settings('breadcrumb_position');
            $dark_bg_breadcrumb = glidex_customizer_settings('enable_dark_bg_breadcrumb');
        ...
        function enqueue_assets() {
            wp_enqueue_style( 'site-breadcrumb', GLIDEX_PLUS_DIR_URL . 'modules/breadcrumb/assets/css/breadcrumb.css', GLIDEX_PLUS_VERSION );
        }
        ...
        function base_meta_style( $meta ) {
            ...
            if( !empty( $style ) ){
                wp_register_style( 'glidex-breadcrumb-inline', '', array (), GLIDEX_PLUS_VERSION, 'all' );
                wp_enqueue_style( 'glidex-breadcrumb-inline' );
                wp_add_inline_style( 'glidex-breadcrumb-inline', $style );
            }
        }

        function register_breadcrumb_params() {

            $enable_delimiter = glidex_customizer_settings( 'change_breadcrumb_delimiter' );
            $delimiter        = glidex_customizer_settings( 'breadcrumb_delimiter' );

            $delimiter = ( $enable_delimiter ) ? '<span class="'.esc_attr($delimiter).'"></span>' : '<span class="breadcrumb-default-delimiter"></span>';

            $wrapper_class    = array();
            $enable_darkbg    = glidex_customizer_settings( 'enable_dark_bg_breadcrumb' );
            $breadcrumb_style = glidex_customizer_settings( 'breadcrumb_style' );
            ...
            $bc_overlay_bg_color = glidex_customizer_settings( 'breadcrumb_overlay_bg_color' );
            if($bc_overlay_bg_color) {
                $wrapper_class[] = 'overlay-wrapper';
            }

            $hide_content    = glidex_customizer_settings( 'hide_content' );
            if( $hide_content ) {
                $wrapper_class[] = 'hide-content';
            }

            $params = array(
                'home'             => esc_html__( 'Home', 'glidex-plus' ),
                'home_link'        => home_url('/'),
                'delimiter'        => $delimiter,
        ...
        function register_meta_params( $post_id ) {
            ...
            $post_meta = get_post_meta( $post_id, '_glidex_breadcrumb_settings', true );
        ...
        function register_template( $args, $post_id ) {
            ...
            if( empty($bc_meta) || ( array_key_exists( 'layout', $bc_meta ) && $bc_meta['layout'] != 'disable' ) ) {
                ...
                if( array_key_exists( 'layout', $bc_meta ) && $bc_meta['layout'] == 'individual-option' ) {
                    ...
                    $enable_bc = glidex_customizer_settings( 'enable_breadcrumb' );
                    if( ! $enable_bc ) {
                        return;
                    }
                }

                $bc_source = glidex_customizer_settings( 'breadcrumb_source' );

                switch( $bc_source ):

                    case 'default':
                    default:
                        glidex_template_part( 'breadcrumb', 'templates/default/title-content', '', $template_args );
                    break;
