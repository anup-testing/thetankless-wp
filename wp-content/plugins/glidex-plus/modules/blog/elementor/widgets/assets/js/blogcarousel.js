( function( $ ) {
	...
	var dtBlogPosts = function($scope, $) {
        const hoverInstance = new wdtWidgetsEnableHoverInit($scope);
        hoverInstance.init();
    ...
    // Function definition for hover functionality
    const wdtWidgetsEnableHoverInit = function ($scope) {
        const $self = this,
            $window = $(window),
        ...
        $self.init = function () {
            if (settings && settings.wdt_enable_hover_class) {
                $self.hoverClassEnableInit();
            }
        };

        $self.hoverClassEnableInit = function () {
            var $get_scope_name = $scope.find('.wdt-posts-list-wrapper').hasClass('wdt-posts-list-wrapper');

            if ($get_scope_name) {
                var $postEntries = $scope.find('.wdt-post-entry');

                $postEntries.eq(2).addClass('wdt-active');



                $postEntries.mouseover(function () {
                    $postEntries.removeClass('wdt-active');
                    $(this).addClass('wdt-active');
                });

            } else {
                console.log('No posts list wrapper found in this scope.');
            }
        };
