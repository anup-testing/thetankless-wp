...
class Elementor_Blog_Posts extends GlidexElementorWidgetBase {
    ...
    public function get_title() {
        return esc_html__('Blog Posts', 'glidex-plus');
    }
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section( 'wdt_section_general', array(
            ...
            $this->add_control( 'query_posts_by', array(
                ...
                'label'   => esc_html__('Query posts by', 'glidex-plus'),
                'default' => 'category',
                'options' => array(
                    'category'  => esc_html__('From Category (for Posts only)', 'glidex-plus'),
                    'ids'       => esc_html__('By Specific IDs', 'glidex-plus'),
                )
            ) );

            $this->add_control( '_post_categories', array(
                'label'       => esc_html__( 'Categories', 'glidex-plus' ),
                'type'        => Controls_Manager:: SELECT2,
                'label_block' => true,
                'multiple'    => true,
                'options'     => $this->glidex_post_categories(),
                'condition'   => array( 'query_posts_by' => 'category' )
            ) );

            $this->add_control( '_post_ids', array(
                'label'       => esc_html__( 'Select Specific Posts', 'glidex-plus' ),
                'type'        => Controls_Manager::SELECT2,
                'label_block' => true,
                'multiple'    => true,
                'options'     => $this->glidex_post_ids(),
                'condition' => array( 'query_posts_by' => 'ids' )
            ) );

            $this->add_control( 'count', array(
                'type'        => Controls_Manager::NUMBER,
                'label'       => esc_html__('Post Counts', 'glidex-plus'),
                'default'     => '5',
                'placeholder' => esc_html__( 'Enter post count', 'glidex-plus' ),
            ) );
            ...
            $this->add_control( 'blog_post_layout', array(
                ...
                'default' => 'entry-grid',
                'options' => array(
                    'entry-grid'  => esc_html__('Grid', 'glidex-plus'),
                    'entry-list'  => esc_html__('List', 'glidex-plus'),
                    'entry-cover' => esc_html__('Cover', 'glidex-plus'),
                )
            ) );
            ...
            $this->add_control( 'blog_post_columns', array(
                ...
                'options' => array(
                    'one-column'        => esc_html__('I Column', 'glidex-plus'),
                    'one-half-column'   => esc_html__('II Columns', 'glidex-plus'),
                    'one-third-column'  => esc_html__('III Columns', 'glidex-plus'),
                    'one-fourth-column' => esc_html__('IV Columns', 'glidex-plus'),
                ),
            ...
            $this->add_control( 'blog_list_thumb', array(
                ...
                'options' => array(
                    'entry-left-thumb'  => esc_html__('Left Thumb', 'glidex-plus'),
                    'entry-right-thumb' => esc_html__('Right Thumb', 'glidex-plus'),
                ),
            ...
            $this->add_control( 'enable_equal_height', array(
                ...
                'label_on'     => esc_html__( 'Yes', 'glidex-plus' ),
                'label_off'    => esc_html__( 'No', 'glidex-plus' ),
                'return_value' => 'yes',
                'default'      => '',
                'condition'    => array( 'blog_post_layout' => array( 'entry-grid', 'entry-cover' ) )
            ) );
            ...
            $this->add_control(
                ...
                array(
                    'label'    => esc_html__('Enable hover class name', 'glidex-plus'),
                    'type'    => Controls_Manager::SWITCHER,
                    'default'      => '',
                    'label_on'     => esc_html__( 'Yes', 'glidex-plus' ),
                    'label_off'    => esc_html__( 'No', 'glidex-plus' ),
            ...
            $content->add_control( 'element_value', array(
                'type'    => Controls_Manager::SELECT,
				'label'   => esc_html__('Element', 'glidex-plus'),
            ...
            $this->add_control( 'blog_readmore_text', array(
                ...
                'default'     => esc_html__('Read More', 'glidex-plus'),
            ) );
            ...
            $this->add_control( 'blog_image_hover_style', array(
                ...
                'description' => esc_html__('Note: Fade, Rotate & Scale Styles will not work for Gallery Sliders.', 'glidex-plus'),
            ) );
            ...
            $this->add_control( 'blog_pagination', array(
                ...
                'default' => 'older_newer',
                'options' => array(
                    ''                => esc_html__('None', 'glidex-plus'),
                    'older_newer'     => esc_html__('Older & Newer', 'glidex-plus'),
                    'numbered'        => esc_html__('Numbered', 'glidex-plus'),
                    'load_more'       => esc_html__('Load More', 'glidex-plus'),
                    'infinite_scroll' => esc_html__('Infinite Scroll', 'glidex-plus'),
                    'carousel'        => esc_html__('Carousel', 'glidex-plus'),
                ),
            ) );

            $this->add_control( 'el_class', array(
                'type'        => Controls_Manager::TEXT,
                'label'       => esc_html__('Extra class name', 'glidex-plus'),
                'description' => esc_html__('Style particular element differently - add a class name and refer to it in custom CSS', 'glidex-plus')
            ) );

        $this->end_controls_section();

		$this->start_controls_section( 'blog_carousel_section', array(
			'label'     => esc_html__( 'Carousel Settings', 'glidex-plus' ),
			'condition' => array( 'blog_pagination' => 'carousel' ),
		) );
			$this->add_control( 'carousel_effect', array(
				'label'       => esc_html__( 'Effect', 'glidex-plus' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Choose effect for your carousel. Slides Per View has to be 1 for Fade effect.', 'glidex-plus' ),
				'default'     => '',
				'options'     => array(
					''     => esc_html__( 'Default', 'glidex-plus' ),
					'fade' => esc_html__( 'Fade', 'glidex-plus' ),
	            ),
	        ) );

			$this->add_responsive_control( 'carousel_slidesperview', array(
				'label'       => esc_html__( 'Slides Per View', 'glidex-plus' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Number slides of to show in view port.', 'glidex-plus' ),
			...
			$this->add_control( 'carousel_loopmode', array(
				'label'        => esc_html__( 'Enable Loop Mode', 'glidex-plus' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('If you wish, you can enable continuous loop mode for your carousel.', 'glidex-plus'),
				'label_on'     => esc_html__( 'yes', 'glidex-plus' ),
				'label_off'    => esc_html__( 'no', 'glidex-plus' ),
				'default'      => '',
				'return_value' => 'true',
			...
			$this->add_control( 'carousel_mousewheelcontrol', array(
				'label'        => esc_html__( 'Enable Mousewheel Control', 'glidex-plus' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('If you wish, you can enable mouse wheel control for your carousel.', 'glidex-plus'),
				'label_on'     => esc_html__( 'yes', 'glidex-plus' ),
				'label_off'    => esc_html__( 'no', 'glidex-plus' ),
				'default'      => '',
				'return_value' => 'true',
			...
			$this->add_control( 'carousel_bulletpagination', array(
				'label'        => esc_html__( 'Enable Bullet Pagination', 'glidex-plus' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('To enable bullet pagination.', 'glidex-plus'),
				'label_on'     => esc_html__( 'yes', 'glidex-plus' ),
				'label_off'    => esc_html__( 'no', 'glidex-plus' ),
				'default'      => '',
				'return_value' => 'true',
			...
			$this->add_control( 'carousel_arrowpagination', array(
				'label'        => esc_html__( 'Enable Arrow Pagination', 'glidex-plus' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('To enable arrow pagination.', 'glidex-plus'),
				'label_on'     => esc_html__( 'yes', 'glidex-plus' ),
				'label_off'    => esc_html__( 'no', 'glidex-plus' ),
				'default'      => '',
				'return_value' => 'true',
			...
			$this->add_control( 'carousel_arrowpagination_type', array(
				'label'       => esc_html__( 'Arrow Type', 'glidex-plus' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Choose arrow pagination type for your carousel.', 'glidex-plus' ),
				'options'     => array(
					''      => esc_html__('Default', 'glidex-plus'),
					'type2' => esc_html__('Type 2', 'glidex-plus'),
				),
			...
			$this->add_control( 'carousel_scrollbar', array(
				'label'        => esc_html__( 'Enable Scrollbar', 'glidex-plus' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('To enable scrollbar for your carousel.', 'glidex-plus'),
				'label_on'     => esc_html__( 'yes', 'glidex-plus' ),
				'label_off'    => esc_html__( 'no', 'glidex-plus' ),
				'default'      => '',
				'return_value' => 'true',
    ...
    protected function render() {
		...
		$out .= '<div class="wdt-posts-list-wrapper '.esc_attr($container_class).' '.esc_attr($el_class).'wdt-post-list-carousel-'.esc_attr($settings['module_id']).'" '.glidex_html_output($media_carousel_attributes_string).'>';
		...
		$warning = esc_html__('No Posts Found','glidex-plus');
		...
		if ( $rposts->have_posts() ) :
            ...
            $template_args = apply_filters( 'glidex_blog_archive_elem_order_params', $template_args );

            // css enqueue
            wp_enqueue_style( 'glidex-plus-blog', GLIDEX_PLUS_DIR_URL . 'modules/blog/assets/css/blog.css', false, GLIDEX_PLUS_VERSION, 'all');

            $pro_active = false;
            $editor_css = '';
            if( defined('GLIDEX_PRO_DIR_PATH') ) {
                $pro_active = true;
            ...
            if ( file_exists( get_theme_file_path('/modules/blog/templates/'.$post_style.'/assets/css/blog-archive-'.$post_style.'.css') ) ) {
                ...
                $file_path = GLIDEX_PLUS_DIR_PATH . 'modules/blog/templates/'.esc_attr($post_style).'/assets/css/blog-archive-'.esc_attr($post_style).'.css';
                if ( file_exists( $file_path ) ) {
                    wp_enqueue_style( 'wdt-blog-archive-'.esc_attr($post_style), GLIDEX_PLUS_DIR_URL . 'modules/blog/templates/'.esc_attr($post_style).'/assets/css/blog-archive-'.esc_attr($post_style).'.css', false, GLIDEX_PLUS_VERSION, 'all');
                    ob_start();
                    include( $file_path );
                    $editor_css .= "\n\n".ob_get_clean();
                }
                ...
                while( $rposts->have_posts() ) :
                    ...
                    $out .= '<div class="'.esc_attr($combine_class.' '.$item_class).'">';
                        $out .= '<article id="post-'.esc_attr($post_ID).'" class="' . implode( ' ', get_post_class( '', $post_ID ) ) . '">';
                            ...
                            $out .= glidex_get_template_part( 'blog', 'templates/'.esc_attr($post_style).'/post', '', $template_args );
                        $out .= '</article>';
                    $out .= '</div>';
                endwhile;
			...
			if( $blog_pagination == 'numbered' ):
				...
				$out .= '<div class="pagination blog-pagination">'.glidex_pagination($rposts).'</div>';

			elseif( $blog_pagination == 'older_newer' ):

				$out .= '<div class="pagination blog-pagination"><div class="newer-posts">'.get_previous_posts_link( '<i class="wdticon-angle-left"></i>'.esc_html__(' Newer Posts', 'glidex-plus') ).'</div>';
				$out .= '<div class="older-posts">'.get_next_posts_link( esc_html__('Older Posts ', 'glidex-plus').'<i class="wdticon-angle-right"></i>', $rposts->max_num_pages ).'</div></div>';
			...
			elseif( $blog_pagination == 'carousel' ):
				...
				$out .= '<div class="wdt-products-pagination-holder">';
					...
					if( $settings['carousel_arrowpagination'] == 'true' ) {
                        ...
                        $out .= '<a href="#" class="wdt-products-arrow-prev wdt-products-arrow-prev-'.esc_attr($settings['module_id']).'">'.esc_html__('Prev', 'glidex-plus').'</a>';
                        $out .= '<a href="#" class="wdt-products-arrow-next wdt-products-arrow-next-'.esc_attr($settings['module_id']).'">'.esc_html__('Next', 'glidex-plus').'</a>';
						$out .= '</div>';
					}
