...
if( !class_exists( 'GlidexPlusBlogPostsWidget' ) ) {
    class GlidexPlusBlogPostsWidget {
        ...
        function register_widgets( $widgets_manager ) {
            require GLIDEX_PLUS_DIR_PATH. 'modules/blog/elementor/widgets/blog-posts/class-widget-blog-posts.php';
        ...
        public function register_call_blog_elementor_sc_filters( $settings ) {
            ...
            add_filter( 'glidex_blog_archive_elem_order_params', array( $this, 'register_blog_archive_elem_order_params' ), 20, 1 );
        }
        ...
        public function blog_elementor_sc_load_more_post() {
            ...
            $blogpostloadmore_nonce = glidex_sanitization($_POST['blogpostloadmore_nonce']);
            if( isset( $blogpostloadmore_nonce ) && wp_verify_nonce( $blogpostloadmore_nonce, 'blogpostloadmore_nonce' ) ) {

                $parse_str = ( isset( $_REQUEST['settings'] ) ) ? parse_str( glidex_sanitization($_REQUEST['settings']), $settings ) : '';
                $this->blog_settings = $settings;

                $count = ( isset( $_REQUEST['count'] ) ) ? glidex_sanitization($_REQUEST['count']) : get_option( 'posts_per_page' );
                $page  = ( isset( $_REQUEST['pageNumber'] ) ) ? glidex_sanitization($_REQUEST['pageNumber']) : 2;
                $cats  = ( isset( $_REQUEST['cats'] ) ) ? explode( ',', glidex_sanitization($_REQUEST['cats']) ) : '';
                    ...
                    while( $the_query->have_posts() ) :
                        ...
                        <div class="<?php echo esc_attr($combine_class);?>">
                            <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>><?php
                                ...
                                glidex_template_part( 'blog', 'templates/'.esc_attr($post_style).'/post', '', $template_args ); ?>
                            </article>
                        </div><?php
                    endwhile;
                    wp_reset_postdata();
        ...
        function register_widget_styles() {
            wp_register_style( 'swiper',
                GLIDEX_PLUS_DIR_URL . 'modules/blog/elementor/widgets/assets/css/swiper.min.css', array(), GLIDEX_PLUS_VERSION );
            wp_register_style( 'wdt-blogcarousel',
                GLIDEX_PLUS_DIR_URL . 'modules/blog/elementor/widgets/assets/css/blogcarousel.css', array(), GLIDEX_PLUS_VERSION );
        }

        function register_widget_scripts() {
            wp_register_script( 'jquery-swiper',
                GLIDEX_PLUS_DIR_URL . 'modules/blog/elementor/widgets/assets/js/swiper.min.js', array( 'jquery' ), GLIDEX_PLUS_VERSION, true );
            wp_register_script( 'wdt-blogcarousel',
                GLIDEX_PLUS_DIR_URL . 'modules/blog/elementor/widgets/assets/js/blogcarousel.js', array(), GLIDEX_PLUS_VERSION, true );
        }
...
GlidexPlusBlogPostsWidget::instance();