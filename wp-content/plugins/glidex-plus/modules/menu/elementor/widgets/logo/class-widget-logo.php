...
class Elementor_Logo extends GlidexElementorWidgetBase {
    ...
    public function get_title() {
        return esc_html__('Logo', 'glidex-plus');
    }
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section( 'wdt_section_general', array(
            ...
            $this->add_control( 'logo_type', array(
				...
				'label'   => esc_html__('Logo Type', 'glidex-plus'),
                'default' => 'theme-logo',
                'options' => array(
                    'theme-logo'  => esc_html__('Logo', 'glidex-plus'),
                    'custom-image'  => esc_html__('Custom Image', 'glidex-plus'),
                    'text' => esc_html__('Title', 'glidex-plus'),
                    'text-desc' => esc_html__('Title and Description', 'glidex-plus'),
                )
            ) );

                $this->add_control( 'theme_logo_type', array(
                    'type'    => Controls_Manager::SELECT,
    				'label'   => esc_html__('Logo', 'glidex-plus'),
                    'default' => 'logo',
                    'options' => array(
                        'logo'  => esc_html__('Logo', 'glidex-plus'),
                        'light-logo'  => esc_html__('Light Logo', 'glidex-plus'),
                    ),
    			...
    			$this->add_control( 'logo_text', array(
    				'label'     => esc_html__( 'Site Title', 'glidex-plus' ),
    			...
    			$this->add_control( 'logo_tagline', array(
    				'label'     => esc_html__( 'Site Tagline', 'glidex-plus' ),
    				'type'      => Controls_Manager::TEXT,
        ...
        $this->start_controls_section( 'wdt_section_color', array(
			...
			$this->add_control( 'site_title_color_info', array(
				...
				'raw' => esc_html__( 'Site Title Color', 'glidex-plus' ),
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
			));

            $this->add_control( 'default_item_color', array(
                'type'    => Controls_Manager::SELECT,
				'label'   => esc_html__('Item Color', 'glidex-plus'),
                'default' => 'none',
                'options' => array(
                    'primary_color'   => esc_html__('Theme Primary', 'glidex-plus'),
                    'secondary_color' => esc_html__('Theme Secondary', 'glidex-plus'),
                    'tertiary_color'  => esc_html__('Theme Tertiary', 'glidex-plus'),
                    'custom'		  => esc_html__('Custom Color', 'glidex-plus'),
                    'none'			  => esc_html__('None', 'glidex-plus')
                ),
                'condition' => array( 'logo_type' => array( 'text', 'text-desc' ) )
            ) );
            ...
            $this->add_control( 'default_bg_color', array(
                'type'    => Controls_Manager::SELECT,
				'label'   => esc_html__('BG Color', 'glidex-plus'),
                'default' => 'none',
                'options' => array(
                    'primary_color'   => esc_html__('Theme Primary', 'glidex-plus'),
                    'secondary_color' => esc_html__('Theme Secondary', 'glidex-plus'),
			...
			$this->add_control( 'site_desc_color_info', array(
				'type' => Controls_Manager::RAW_HTML,
				'raw' => esc_html__( 'Site Description Color', 'glidex-plus' ),
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
				'condition' => array( 'logo_type' => array( 'text-desc' ) )
			));
        ...
        $this->start_controls_section( 'wdt_section_typhography', array(
            'label' => esc_html__( 'Typography', 'glidex-plus'),
        	'condition' => array( 'logo_type' => array( 'text', 'text-desc' ) )
        ) );

            $this->add_control( 'use_theme_fonts', array(
                'type'         => Controls_Manager::SWITCHER,
                'label'        => esc_html__('Use theme default font family?', 'glidex-plus'),
            ...
            $this->add_control( 'use_theme_fonts_desc', array(
 			...
 			$this->add_group_control( Group_Control_Typography::get_type(), array(
                'label'     => esc_html__('Site Description', 'glidex-plus'),
				'name'      => 'site_desc_typo',
    ...
    protected function render() {
        ...
        if($_element_id != '') {
            $el_id = 'glidex-'.esc_attr($_element_id);
        } else {
        	$el_id = 'glidex-'.esc_attr($this->get_id());
        }
        ...
        # CUSTOM CSS
        $custom_css = '';
        $custom_css .= $this->glidex_generate_css( $settings );
        ...
        if( $logo_type == 'theme-logo' ) {
            if( $theme_logo_type == 'logo' ) {
                ...
                $alogo = glidex_customizer_settings( 'custom_alt_logo' );
                ...
                if( empty( $url ) ) {
                    $url = GLIDEX_ROOT_URI.'/assets/images/light-logo.svg';
                }
            }
        ...
        $output .= '  <a href="'.esc_url( home_url( '/' ) ).'" rel="home">'.glidex_html_output($logo).'</a>';
        $output .= '</div>';

        if( !empty( $custom_css ) ) {
            $this->glidex_print_css( $custom_css );
        }

        echo glidex_html_output($output);
	}

	function glidex_generate_css( $attrs ) {

        $css = $breakpoint_css = '';

        if(isset( $attrs['_element_id'] ) && $attrs['_element_id'] != '') {
            $attrs['el_id'] = 'glidex-'.esc_attr($attrs['_element_id']);
        } else {
        	$attrs['el_id'] = 'glidex-'.esc_attr($this->get_id());
        }
        ...
        if( ( $attrs['logo_type'] == 'text' || $attrs['logo_type'] == 'text-desc' ) && !empty( $attrs['logo_text'] ) ) {
            ...
            if( $attrs['default_item_color'] == 'custom' &&  !empty( $attrs['default_custom_item_color'] ) ) {
                ...
                $t_color = $this->glidex_current_skin( $attrs['default_item_color'] );
            }
            ...
            if( $attrs['default_bg_color'] == 'custom' &&  !empty( $attrs['default_custom_bg_color'] ) ) {
                ...
                $t_bg_color = $this->glidex_current_skin( $attrs['default_bg_color'] );
            }
            ...
            if( $attrs['default_border_color'] == 'custom' &&  !empty( $attrs['default_custom_border_color'] ) ) {
                ...
                $t_border_color = $this->glidex_current_skin( $attrs['default_border_color'] );
            }
        ...
        if( $attrs['logo_type'] == 'text-desc' && !empty( $attrs['logo_tagline'] ) ) {
            ...
            if( $attrs['desc_default_item_color'] == 'custom' &&  !empty( $attrs['desc_default_custom_item_color'] ) ) {
                ...
                $t_color = $this->glidex_current_skin( $attrs['desc_default_item_color'] );
            }
            $font_style .= ( !empty( $t_color ) ) ? 'color:'.esc_attr($t_color).';' : '';
            ...
            if( $attrs['desc_default_bg_color'] == 'custom' &&  !empty( $attrs['desc_default_custom_bg_color'] ) ) {
                ...
                $t_bg_color = $this->glidex_current_skin( $attrs['desc_default_bg_color'] );
            }
            $font_style .= ( !empty( $t_bg_color ) ) ? 'background-color:'.esc_attr($t_bg_color).'; padding:4px;' : '';
            ...
            if( $attrs['desc_default_border_color'] == 'custom' &&  !empty( $attrs['desc_default_custom_border_color'] ) ) {
                ...
                $t_border_color = $this->glidex_current_skin( $attrs['desc_default_border_color'] );
            }
            $font_style .= ( !empty( $t_border_color ) ) ? 'border-style:solid; border-width:1px; border-color:'.esc_attr($t_border_color).'; ' : '';
    ...
    function glidex_print_css( $css ) {

        if( !empty( $css ) ) {
            wp_register_style( 'wdt-elementor-logo-inline', '', array (), GLIDEX_PLUS_VERSION, 'all' );
            wp_enqueue_style( 'wdt-elementor-logo-inline' );
            wp_add_inline_style( 'wdt-elementor-logo-inline', $css );
        }
    }

    function glidex_current_skin( $code = 'primary_color' ) {

        $color = '';
        $mode  = glidex_customizer_settings( 'use_predefined_skin' );

        if( $mode ) {
            $skin  = glidex_customizer_settings( 'predefined_skin' );
            $skin  = glidex_customizer_settings( $skin );
            $color = $skin[$code];
        } else {
            $color = glidex_customizer_settings( $code );
        }
