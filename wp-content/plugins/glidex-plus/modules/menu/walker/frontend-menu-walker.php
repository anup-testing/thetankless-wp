<?php
if ( ! class_exists( 'GlidexPlusFrontendMenuWalker' ) ) {

	class GlidexPlusFrontendMenuWalker {
		...
		function enqueue_css_assets() {
			wp_enqueue_style( 'dtplugin-nav-menu-animations', GLIDEX_PLUS_DIR_URL . 'modules/menu/assets/css/nav-menu-animations.css', false, GLIDEX_PLUS_VERSION, 'all');
			wp_enqueue_style( 'dtplugin-nav-menu', GLIDEX_PLUS_DIR_URL . 'modules/menu/assets/css/nav-menu.css', false, GLIDEX_PLUS_VERSION, 'all');
		}

		function enqueue_js_assets() {
			wp_enqueue_script( 'dtplugin-mega-menu', GLIDEX_PLUS_DIR_URL . 'modules/menu/assets/js/mega-menu.js', array(), GLIDEX_PLUS_VERSION, true );
		}
	}
}

GlidexPlusFrontendMenuWalker::instance();

if( ! class_exists( 'Glidex_Walker_Nav_Menu' ) ) {

	class Glidex_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			$item_output .= '<a' . glidex_html_output($attributes) . '>';
				...
				if( !empty( $item->icon ) || !empty( $item->image ) ) {
					...
					if( !empty( $item->image ) ) {
						$item_output .= '<i class="menu-item-image">';
							$item_output .= '<img src="'.esc_url( $item->image ).'" alt="'.esc_attr( 'Image', 'glidex-plus' ).'"/>';
						$item_output .= '</i>';
					}
				}
