...
if( !function_exists('glidex_plus_get_template_plugin_part') ) {
    function glidex_plus_get_template_plugin_part( $file_path, $module, $template, $slug ) {

        $html             = '';
        $template_path    = GLIDEX_PLUS_DIR_PATH . 'modules/' . esc_attr($module);
    ...
    add_filter( 'glidex_get_template_plugin_part', 'glidex_plus_get_template_plugin_part', 10, 4 );
}

if( !function_exists( 'glidex_customizer_panel_priority' ) ) {
    /**
     *  Get : Customizer Panel Priority based on panel name.
     */
    function glidex_customizer_panel_priority( $panel ) {
        $priority = 10;
        ...
        return apply_filters( 'glidex_customizer_panel_priority', $priority, $panel );
    }
}

if( !function_exists( 'glidex_customizer_settings' ) ) {
    /**
     * Get : Customizer settings value
     */
    function glidex_customizer_settings( $option ) {
        $settings = get_option( GLIDEX_CUSTOMISER_VAL, array() );
        $settings = isset( $settings[ $option ] ) ? $settings[ $option ] : false;
        return $settings;
    }
}

if( !function_exists( 'glidex_customizer_dynamic_style') ) {
    /**
     * Get : Generate style based on selector and property
     */
    function glidex_customizer_dynamic_style( $selectors, $properties ) {
        $output = '';
        if( !empty( $selectors ) && !empty( $properties ) ) {
            ...
            $output .= ' { ' . glidex_html_output($properties) . ' } ' . "\n";
        }
        return $output;
    }
}
...
if( !function_exists( 'glidex_customizer_responsive_typography_settings' ) ) {

    /**
     * Get : Typography Responsive CSS based on option and responsive mode.
     */
    function glidex_customizer_responsive_typography_settings( $option, $mode = 'tablet' ) {
        $css = '';
...
if( !function_exists( 'glidex_customizer_typography_settings' ) ) {
    /**
     * Get : Typography CSS based on option.
     */
    function glidex_customizer_typography_settings( $option ) {
...
if( !function_exists( 'glidex_customizer_frontend_font' ) ) {
    /**
     * Load fonts in frontend
     */
    function glidex_customizer_frontend_font( $settings, $fonts ) {
        $font = '';
...
if( !function_exists( 'glidex_customizer_color_settings' ) ) {
    function glidex_customizer_color_settings( $color ) {
        $css = '';
...
if( !function_exists( 'glidex_customizer_bg_color_settings' ) ) {
    function glidex_customizer_bg_color_settings( $color ) {
        $css = '';
...
if( !function_exists( 'glidex_customizer_bg_settings' ) ) {
    function glidex_customizer_bg_settings( $bg ) {
        $css = '';
...
# Field Sanitization
if(!function_exists('glidex_sanitization')) {
	function glidex_sanitization($data) {
		if ( is_array( $data ) && !empty( $data ) ) {
			foreach ( $data as $key => &$value ) {
				if ( is_array( $value ) ) {
					$data[$key] = glidex_sanitization($value);
				} else {
