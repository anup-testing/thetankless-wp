	...
	<?php else : ?>
		<div class="wpo-fieldgroup">
			<p id="wpo_gzip_compression_status" class="<?php echo esc_attr($class_name); ?>">
				...
				<?php if ($wp_optimize->is_apache_server() && $wpo_gzip_compression_partially_enabled) : ?>
					<div>
					<?php
						// translators: %1$s and %2$s is a list of content types HTML, CSS, JS.
						printf(esc_html__('Gzip is already enabled for %1$s files but you can make loading faster by enabling gzip compression with WP-Optimize for %2$s and other static files', 'wp-optimize'), esc_html(join('/', $wpo_gzip_compression_content_types['enabled'])), esc_html(join(', ', $wpo_gzip_compression_content_types['disabled'])));
					?>
					</div>
				<?php endif; ?>
			</p>
			...
			if (is_wp_error($wpo_gzip_compression_enabled) || false === $wpo_gzip_compression_enabled || $wpo_gzip_compression_settings_added) {
				...
				if ($wp_optimize->is_apache_server()) {
					$button_text = $wpo_gzip_compression_settings_added ? __('Disable', 'wp-optimize') : __('Enable', 'wp-optimize');
					?>
					<form>
						<button class="button-primary" type="button"
							id="wp_optimize_gzip_compression_enable" data-enable="<?php echo $wpo_gzip_compression_settings_added ? '0' : '1'; ?>"><?php echo esc_html($button_text); ?></button>
