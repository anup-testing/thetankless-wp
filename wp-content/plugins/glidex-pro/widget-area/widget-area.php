...
if( !class_exists( 'GlidexProWidgetArea' ) ) {
    class GlidexProWidgetArea {
        ...
        function __construct() {
             ...
             * Before Hook
             */
            do_action( 'glidex_pro_before_fw_widget_area_load' );
                ...
                add_action( 'wp_ajax_glidex_pro_delete_widget_area', array( $this, 'delete_widget_area' ) );

            /**
             * Adter Hook
             */
            do_action( 'glidex_pro_after_fw_metabox_load' );
        }
				...
				<div id="wdt-pro-add-widget" class="widgets-holder-wrap">
					<div>
						<div class="sidebar-name">
							<h3><?php esc_html_e( 'Create Widget Area', 'glidex-pro' ); ?>
			  			...
			  			<div class="sidebar-description">
			  				<form method="post">
			  					<div class="widget-content">
			  						<input id="wdt-pro-add-widget-input" name="wdt-pro-add-widget-input" type="text" class="regular-text" title="<?php esc_attr_e( 'Name', 'glidex-pro' ); ?>" placeholder="<?php esc_attr_e( 'Name', 'glidex-pro' ); ?>" />
			  					</div>
			  					<div class="widget-control-actions">
			  						<div class="aligncenter">
			  							<input class="button-primary" type="submit" value="<?php esc_attr_e( 'Create Widget Area', 'glidex-pro' ); ?>" />
			  						</div>
									<br class="clear">
								</div>
							</form>
        ...
        public function enqueue_scripts() {
            wp_enqueue_script( 'glidex-widget-areas', GLIDEX_PRO_DIR_URL.'widget-area/assets/js/widget-areas.js', array('jquery'), GLIDEX_PRO_VERSION, true );
            wp_enqueue_style( 'glidex-widget-areas', GLIDEX_PRO_DIR_URL.'widget-area/assets/css/widget-areas.css', null, GLIDEX_PRO_VERSION );
            wp_localize_script( 'glidex-widget-areas', 'wdtProObject', array(
				'delete'      => esc_html__( 'Delete', 'glidex-pro' ),
				'confirm'     => esc_html__( 'Confirm', 'glidex-pro' ),
				'cancel'      => esc_html__( 'Cancel', 'glidex-pro' ),
				'deleteNonce' => wp_create_nonce('widgetarea_delete_nonce')
			) );
        }

		public function create_widget_area() {
			$add_widget_input = isset($_POST['wdt-pro-add-widget-input']) ? glidex_sanitization($_POST['wdt-pro-add-widget-input']) : '';
		...
		public function get_widget_areas() {
            ...
            $widget_areas = get_option( 'glidex-widget-areas' );
            if( $widget_areas ) {
		...
		public function save_widget_areas() {
			$options = get_option( 'glidex-widget-areas', array() );
			...
			update_option( 'glidex-widget-areas', $options );
        }

        public function register_main_widget_areas() {
            $sidebars = apply_filters( 'glidex_register_sidebars_array', array() );
            if ( $sidebars ) {
				$before_title = apply_filters( 'glidex_widget_before_title_tag', '<h2 class="widgettitle">' );
                $after_title  = apply_filters( 'glidex_widget_after_title_tag', '</h2>' );
        ...
        public function delete_widget_area() {
			$widgetarea_delete_nonce = glidex_sanitization($_POST['widgetarea_delete_nonce']);
			if(isset($widgetarea_delete_nonce) && wp_verify_nonce($widgetarea_delete_nonce, 'widgetarea_delete_nonce')) {
				$widget_id = glidex_sanitization($_REQUEST['widget']);
				if( !empty( $widget_id ) ) {
