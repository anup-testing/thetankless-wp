...
class CSFramework_Option_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    if( is_object( $google_json ) ) {
      ...
      echo '<select name="'. esc_attr($this->element_name( '[family]' )) .'" class="'. esc_attr($is_chosen) . $chosen_rtl .'cs-typo-family" data-atts="family"'. glidex_html_output($this->element_attributes()) .'>';

      do_action( 'cs_typography_family', $family_value, $this );

      echo '<optgroup label="'. esc_attr__( 'Web Safe Fonts', 'glidex-pro' ) .'">';
      ...
      echo '<optgroup label="'. esc_attr__( 'Google Fonts', 'glidex-pro' ) .'">';
      ...
      if( ! empty( $is_variant ) ) {
        ...
        foreach ( $variants as $variant ) {
          echo '<option value="'. esc_attr($variant) .'"'. glidex_html_output($this->checked( $variant_value, $variant, 'selected' )) .'>'. $variant .'</option>';
        }
        echo '</select>';
        echo '</label>';
      ...
      echo esc_html__( 'Error! Can not load json file.', 'glidex-pro' );
