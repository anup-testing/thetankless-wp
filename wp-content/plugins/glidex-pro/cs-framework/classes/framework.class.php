...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'glidex-pro' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'glidex-pro' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'glidex-pro' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    echo '<div class="cs-framework cs-option-framework">';
      ...
      if( $this->settings['ajax_save'] ) {
        ...
        echo '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.', 'glidex-pro' ) .'</span>';
      }

      submit_button( esc_html__( 'Save', 'glidex-pro' ), 'primary cs-save', 'save', false, array( 'data-save' => esc_html__( 'Saving...', 'glidex-pro' ) ) );
      submit_button( esc_html__( 'Restore', 'glidex-pro' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );

      if( $this->settings['show_reset_all'] ) {
        submit_button( esc_html__( 'Reset All Options', 'glidex-pro' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }

      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'glidex-pro' ) .'</a>' : '';
        ...
        echo '<div class="cs-content">';
          ...
          foreach( $this->sections as $section ) {
            ...
            if( isset( $section['fields'] ) ) {
              ...
              echo '<div id="cs-tab-'. $section['name'] .'" class="cs-section"'. glidex_html_output($active_content) .'>';
