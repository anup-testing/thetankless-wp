...
if( !class_exists( 'GlidexProSidebar' ) ) {
    class GlidexProSidebar {
        ...
        function __construct() {
            ...
            $this->global_layout  = glidex_customizer_settings('global_sidebar_layout');
            $this->global_sidebar = glidex_customizer_settings('global_sidebar');
            $this->hide_standard_sidebar = glidex_customizer_settings('hide_standard_sidebar');
        ...
        function load_modules() {
            include_once GLIDEX_PRO_DIR_PATH.'modules/sidebar/customizer/index.php';
            include_once GLIDEX_PRO_DIR_PATH.'modules/sidebar/metabox/index.php';
        }

        function frontend() {
            add_action('glidex_after_main_css', array( $this, 'enqueue_assets' ) );
            add_filter('glidex_primary_classes', array( $this, 'primary_classes' ), 20 );
            add_filter('glidex_secondary_classes', array( $this, 'secondary_classes' ), 20 );
            add_filter('glidex_active_sidebars', array( $this, 'active_sidebars' ), 20 );
        }

        function enqueue_assets() {
            wp_enqueue_style( 'site-sidebar', GLIDEX_PRO_DIR_URL . 'modules/sidebar/assets/css/sidebar.css', false, GLIDEX_PRO_VERSION, 'all' );
            $id       = get_the_ID();
            $settings = get_post_meta( $id, '_glidex_layout_settings', true );
            ...
            if( ( isset($settings['sticky_sidebar'] ) && !empty($settings['sticky_sidebar'] ) ) || ( isset($settings['layout'] ) && !empty($settings['layout'] ) && $settings['layout'] == 'global-sidebar-layout' ) ) {
                wp_enqueue_style( 'sidebar', GLIDEX_PRO_DIR_URL . 'modules/sidebar/assets/css/sidebar.css', false, GLIDEX_PRO_VERSION, 'all' );
                wp_enqueue_script( 'theia-sticky-sidebar', GLIDEX_PRO_DIR_URL . 'assets/js/theia-sticky-sidebar.min.js', array('jquery'), GLIDEX_PRO_VERSION, true );
                wp_enqueue_script( 'sidebar-sticky', GLIDEX_PRO_DIR_URL . 'modules/sidebar/assets/js/side-bar.js', array('theia-sticky-sidebar'), GLIDEX_PRO_VERSION, true );
            }
        }
        ...
        function primary_classes( $primary_class ) {
            ...
            if( is_singular( $this->sidebar_post_types )  ) {
                $settings = get_post_meta( get_queried_object_id(), '_glidex_layout_settings', true );
                $settings = is_array( $settings ) ? array_filter( $settings ) : array();
                ...
                if( isset( $settings['layout'] ) ) {
                    if( $settings['layout'] == 'global-sidebar-layout' ) {
                        ...
                        if($this->hide_standard_sidebar) {
                            unset($sidebars[array_search('glidex-standard-sidebar-1', $sidebars)]);
                        }
                    } else {
