    ...
    <div class="side-nav-container">
        <ul class="side-nav"><?php
            ...
            foreach( $ids as $pid ) {
                ...
                $settings = get_post_meta( $pid, '_glidex_sidenav_settings', true );
    ...
    if( !empty( $show_content ) && !empty( $content_id ) ){
        if( class_exists( '\Elementor\Plugin' ) ) {
            ...
            if( !empty( $elementor_instance ) ) {
                echo glidex_html_output($elementor_instance->frontend->get_builder_content_for_display( $content_id ));
            }
