...
class Elementor_Side_Nav_Navigation extends GlidexElementorWidgetBase {
    ...
    public function get_title() {
        return esc_html__('Side Nav - Navigation', 'glidex-pro');
    }
        ...
        $output .= '<div class="wdt-sidenav-navigation-container '.esc_attr( $el_class ).'">';
            ...
            if($prev_page_id >= 0) {
                ...
                $output .= '<div class="wdt-sidenav-navigation-prev-wrapper">';
                    ...
                    $output .= '<div class="wdt-sidenav-navigation-title-wrapper">';
                        $output .= '<p><a href="'.get_permalink($prev_page_id).'" title="'.esc_attr(get_the_title($prev_page_id)).'">'.esc_html__('Previous Page','glidex-pro').'</a></p>';
                        ...
                        $output .= '<h3><a href="'.get_permalink($prev_page_id).'" title="'.esc_attr(get_the_title($prev_page_id)).'">';
                            if(get_the_title($prev_page_id)) {
                                ...
                                $output .= esc_html__('Previous Page', 'glidex-pro');
                            }
                        $output .= '</a></h3>';
                    $output .= '</div>';
                $output .= '</div>';
                ...
                $output .= '<div class="wdt-sidenav-navigation-prev-wrapper no-post">';
                    $output .= '<a href="#" style="background-image:url('.esc_url(GLIDEX_ROOT_URI.'/assets/images/no-post.jpg').');" class="wdt-sidenav-navigation-prev-bgimg"></a>';
                    $output .= '<div class="wdt-sidenav-navigation-title-wrapper">';
                        $output .= '<span class="zmdi zmdi-long-arrow-left zmdi-hc-fw"></span>';
                        $output .= '<h3>'.esc_html__('No previous page to show!', 'glidex-pro').'</h3>';
                    $output .= '</div>';
                $output .= '</div>';
            ...
            if($next_page_id >= 0) {
                ...
                $output .= '<div class="wdt-sidenav-navigation-next-wrapper">';
                    ...
                    $output .= '<div class="wdt-sidenav-navigation-title-wrapper">';
                        $output .= '<p><a href="'.get_permalink($next_page_id).'" title="'.esc_attr(get_the_title($next_page_id)).'">'.esc_html__('Next Page','glidex-pro').'</a></p>';
                        $output .= '<span class="zmdi zmdi-long-arrow-left zmdi-hc-fw"></span>';
                        $output .= '<h3><a href="'.get_permalink($next_page_id).'" title="'.esc_attr(get_the_title($next_page_id)).'">';
                            if(get_the_title($next_page_id)) {
                                ...
                                $output .= esc_html__('Next Page', 'glidex-pro');
                            }
                        $output .= '</a></h3>';
                    $output .= '</div>';
                $output .= '</div>';
                ...
                $output .= '<div class="wdt-sidenav-navigation-next-wrapper no-post">';
                    $output .= '<a href="#" style="background-image:url('.esc_url(GLIDEX_ROOT_URI.'/assets/images/no-post.jpg').');"  class="wdt-sidenav-navigation-next-bgimg"></a>';
                    $output .= '<div class="wdt-sidenav-navigation-title-wrapper">';
                        $output .= '<span class="zmdi zmdi-long-arrow-left zmdi-hc-fw"></span>';
                        $output .= '<h3>'.esc_html__('No next page to show!', 'glidex-pro').'</h3>';
                    $output .= '</div>';
                $output .= '</div>';
