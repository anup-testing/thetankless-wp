...
if( !class_exists( 'GlidexProSideNav' ) ) {
    class GlidexProSideNav {
        ...
        function load_module() {
            include_once GLIDEX_PRO_DIR_PATH.'modules/side-nav/metabox/index.php';
            include_once GLIDEX_PRO_DIR_PATH.'modules/side-nav/elementor/index.php';
            add_filter( 'theme_page_templates', array( $this, 'add_sidenav_template' ) );
        }

        function add_sidenav_template( $templates ) {
            $templates = array_merge( $templates, array( 'tpl-side-nav.php' => esc_html__('Side Navigation', 'glidex-pro' ) ) );
            return $templates;
        }

        function frontend() {
            add_action( 'glidex_after_main_css', array( $this, 'enqueue_assets' ) );
            add_action( 'glidex_before_single_page_content_wrap', array( $this, 'before_content_wrap' ) );
            add_action( 'glidex_after_single_page_content_wrap', array( $this, 'after_content_wrap' ), 9999999 );
        }

        function enqueue_assets() {
            $page_template = $this->get_page_template();
            if( $page_template == 'tpl-side-nav.php' ) {
                wp_enqueue_style( 'sidenav', GLIDEX_PRO_DIR_URL . 'modules/side-nav/assets/css/sidenav.css', false, GLIDEX_PRO_VERSION, 'all' );
                ...
                wp_enqueue_script( 'sidenav-sticky', GLIDEX_PRO_DIR_URL . 'modules/side-nav/assets/js/side-nav.js', array('theia-sticky-sidebar'), GLIDEX_PRO_VERSION, true );
            }
        }
        ...
        function before_content_wrap() {
            ...
            if( $page_template == 'tpl-side-nav.php' ) {
                $id       = get_the_ID();
                $settings = get_post_meta( $id, '_glidex_sidenav_settings', true );
                ...
                echo '<div class="side-navigation-container '.esc_attr($settings['style']).'">';
                    echo glidex_get_template_part( 'side-nav', 'template/tpl-side-nav', '', array( 'id' => $id, 'show_content' => $show_content, 'content_id' => $content_id, 'classes' => $classes ) );
                ...
                if( !empty( $show_bottom_content ) && !empty( $bottom_content_id ) ){
                    if( class_exists( '\Elementor\Plugin' ) ) {
                        ...
                        if( !empty( $elementor_instance ) ) {
                            echo glidex_html_output($elementor_instance->frontend->get_builder_content_for_display( $bottom_content_id ));
                        }
