<?php
class Glidex_Widget_Recent_Posts extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			'classname'   => 'widget_recent_posts',
			'description' => esc_html__('To list out posts', 'glidex-pro')
		);

        $theme_name =  defined('GLIDEX_THEME_NAME') ? GLIDEX_THEME_NAME : 'Glidex';
		parent::__construct( false, $theme_name . esc_html__(' Posts','glidex-pro'), $widget_options );
	}
	...
	function form($instance) {
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
        		<?php esc_html_e('Title:','glidex-pro');?>
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_categories')); ?>">
	    		<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','glidex-pro');?>
	    	</label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('_post_categories').'[]');?>" name="<?php echo esc_attr($this->get_field_name('_post_categories').'[]');?>" multiple="multiple">
            	<option value=""><?php esc_html_e("Select",'glidex-pro');?></option><?php
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>">
        		<?php esc_html_e('Display title only or title &amp; excerpt','glidex-pro');?>
        	</label>
        ...
        <p>
        	...
        	<?php esc_html_e("Show Image",'glidex-pro');?>
        </p>

	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_count'));?>">
	    		<?php esc_html_e('No.of posts to show:','glidex-pro');?>
	    	</label>
	...
	function widget($args, $instance) {
		...
		echo glidex_pro_before_after_widget( $before_widget );

		if( !empty( $title ) ) {

			echo glidex_pro_widget_title( $before_title . $title . $after_title );
		}
		...
		echo "<div class='recent-posts-widget'><ul>";
			 ...
			 while( $the_query->have_posts() ):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						...
						$image = ( $image != false)? $image[0] : GLIDEX_PRO_DIR_URL . 'modules/blog/assets/images/post-thumb.jpg';
					...
					if($show_title):
						...
						echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
						echo "<div class='entry-content'>".glidex_excerpt(10)."</div>";
					endif;
			 ...
			 endwhile;
			 	...
			 	echo "<li><h4>".esc_html__('No Posts found','glidex-pro')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";

		echo glidex_pro_before_after_widget( $after_widget );
	}
}?>