...
<?php
	...
	<div class="entry-thumb">
        ...
        <div class="entry-thumb-detail-group">
            <?php if(in_array('author', $archive_post_elements)) :?>
                <div class="entry-author">
                    <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" title="<?php esc_attr_e('View all posts by ', 'glidex-pro'); echo get_the_author();?>">
                        <?php echo get_the_author();?>
                    </a>
                    <span><?php esc_html_e('wrote', 'glidex-pro'); ?></span>
                </div>
            <?php endif; ?>
            <?php if(in_array('title', $archive_post_elements)) :?>
                <div class="entry-title">
                    <h4><?php
                        if( is_sticky( $post_ID ) ) echo '<span class="sticky-post"><i class="wdticon-thumb-tack"></i><span>'.esc_html__('Featured', 'glidex-pro').'</span></span>'; ?>
                        <a href="<?php echo get_permalink( $post_ID );?>" title="<?php printf(esc_attr__('Permalink to %s','glidex-pro'), the_title_attribute('echo=0'));?>"><?php the_title();?></a>
                    </h4>
                </div>
            <?php endif; ?>
                ...
                <div class="entry-thumb-group">
                    ...
                    <?php if(in_array('category', $archive_post_elements)) :?>
                        <div class="entry-categories">
                            <span><?php esc_html_e('in', 'glidex-pro'); ?></span>
                            <?php
        ...
        <?php do_action( 'glidex_blog_archive_post_format', $enable_post_format, $post_format ); ?>