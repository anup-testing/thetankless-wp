...
<table class="shop_table cart wishlist_table wishlist_view traditional responsive" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr( $wishlist_id ); ?>" data-token="<?php echo esc_attr( $wishlist_token ); ?>">
    ...
    <tr>
        ...
        <th class="product-thumbnail">
            <span class="nobr">
                <?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product', 'glidex-pro' ) ) ?>
            </span>
        </th>
        ...
        <?php if( $show_price || $show_price_variations ) : $column_count ++; ?>
            <th class="product-price">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit price', 'glidex-pro' ) ) ?>
                </span>
            </th>
        <?php endif; ?>
        ...
        <?php if( $show_quantity ) : $column_count ++; ?>
            <th class="product-quantity">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', esc_html__( 'Quantity', 'glidex-pro' ) ) ?>
                </span>
            </th>
        <?php endif;?>
        ...
        <?php if( $show_stock_status ) : $column_count ++; ?>
            <th class="product-stock-status">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock status', 'glidex-pro' ) ) ?>
                </span>
            </th>
        <?php endif; ?>
        ...
        <?php if( $enable_drag_n_drop ): $column_count ++; ?>
            <th class="product-arrange">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', esc_html__( 'Arrange', 'glidex-pro' ) )?>
                </span>
            </th>
        <?php endif; ?>
    ...
    if( $wishlist && $wishlist->has_items() ) :
                ...
                <tr id="yith-wcwl-row-<?php echo esc_attr($item->get_product_id()); ?>" data-row-id="<?php echo esc_attr($item->get_product_id()); ?>">
                    ...
                    <td class="product-thumbnail">
                        <a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ) ?>">
                            <?php echo glidex_html_output($product->get_image()); ?>
                        </a>
                    </td>
                    ...
                    <?php if( $show_price || $show_price_variations ) : ?>
                        <td class="product-price">
                            <?php
                            if( $show_price ) {
                                echo glidex_html_output($item->get_formatted_product_price());
                            }

                            if( $show_price_variations ){
                                echo glidex_html_output($item->get_price_variation());
                            }
                            ?>
                        </td>
                    <?php endif ?>
                    ...
                    <?php if( $show_quantity ) : ?>
                        <td class="product-quantity">
                            ...
                            <?php else: ?>
                                <?php echo glidex_html_output($item->get_quantity()); ?>
                            <?php endif; ?>
                        </td>
                    <?php endif;?>
                    ...
                    <?php if( $show_stock_status ) : ?>
                        <td class="product-stock-status">
                            <?php echo esc_attr($stock_status) == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . apply_filters( 'yith_wcwl_out_of_stock_label', esc_html__( 'Out of stock', 'glidex-pro' ) ) . '</span>' : '<span class="wishlist-in-stock">' . apply_filters( 'yith_wcwl_in_stock_label', esc_html__( 'In Stock', 'glidex-pro' ) ) . '</span>'; ?>
                        </td>
                    <?php endif ?>
                    ...
                    <?php if( $show_last_column ): ?>
                        <td class="product-add-to-cart">
                            ...
                            if( $show_dateadded && $item->get_date_added() ):
                                echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on: %s', 'glidex-pro' ), $item->get_date_added_formatted() ) . '</span>';
                            endif;
                            ?>
                            ...
                            <?php if( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ): ?>
                                <?php if( 'select' == $move_to_another_wishlist_type ): ?>
                                    <select class="change-wishlist selectBox">
                                        <option value=""><?php _e( 'Move', 'glidex-pro' ) ?></option>
                                        <?php
                                ...
                                <?php else: ?>
                                    <a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
                                        <?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', esc_html__( 'Move to another list &rsaquo;', 'glidex-pro' ) ) ?>
                                    </a>
                                <?php endif; ?>
                            <?php endif; ?>
                            ...
                            <?php if( $repeat_remove_button ): ?>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove_from_wishlist button" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title',esc_html__( 'Remove this product', 'glidex-pro' )); ?>"><?php _e( 'Remove', 'glidex-pro' ) ?></a>
                            <?php endif; ?>
                        </td>
                    <?php endif; ?>
                    ...
                    <?php if( $show_remove_product ): ?>
                        <td class="product-remove">
                            <div>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'glidex-pro' ) ); ?>">&times;</a>
                            </div>
                        </td>
                    <?php endif; ?>
        ...
        <tr>
            <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'glidex-pro' ) ) ?></td>
        </tr>
    <?php
    endif;
    ...
    if( ! empty( $page_links ) ) : ?>
        <tr class="pagination-row">
            <td colspan="<?php echo esc_attr( $column_count ) ?>"><?php echo glidex_html_output($page_links) ?></td>
        </tr>
    <?php endif ?>
    </tbody>
