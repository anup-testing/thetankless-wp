...
if( !class_exists( 'Glidex_Shop_Others_Suggested_Products_Include' ) ) {

    class Glidex_Shop_Others_Suggested_Products_Include {
        ...
        function __construct() {
			add_action( 'glidex_hook_bottom', array( $this, 'glidex_shop_suggested_products_content' ) );
		}

		/**
         * Suggested Products Content
         */
			function glidex_shop_suggested_products_content() {
							...
							while ( $form_query->have_posts() ) {
								...
								echo '<li class="suggested-product-data">';
									echo '<div class="suggested-product-image">';
										...
										echo '<a href="'.esc_url($product_permalink).'"><img src="'.esc_url($image[0]).'" alt="'.esc_html__('Suggest Product Thumbnail', 'glidex-pro').'"></a>';
									echo '</div>';

									echo '<div class="suggested-product-content">';
										echo '<span>'.esc_html__('Someone purchased a ', 'glidex-pro').'</span>';
										echo '<a href="'.esc_url($product_permalink).'">'. esc_html( $product_title ). '</a>';
										echo '<p class="suggested-product-time">'. esc_html( $suggested_products_time ). esc_html__(' Minutes ago from ', 'glidex-pro') .esc_html( $suggested_products_location ).'</p>';
									echo '</div>';
...
if( !function_exists('glidex_shop_others_suggested_products_include') ) {
	function glidex_shop_others_suggested_products_include() {
        $reflection = new ReflectionClass('Glidex_Shop_Others_Suggested_Products_Include');
        return $reflection->newInstanceWithoutConstructor();
	}
}

Glidex_Shop_Others_Suggested_Products_Include::instance();