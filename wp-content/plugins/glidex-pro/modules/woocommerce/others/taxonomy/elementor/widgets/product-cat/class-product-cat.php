...
class Glidex_Shop_Widget_Product_Cat extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Product Categories', 'glidex-pro' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'product_cat_section', array(
			...
			$this->add_control( 'type', array(
				...
				'description' => esc_html__( 'Choose type that you like yo use.', 'glidex-pro' ),
				'options'     => array(
			...
			$this->add_control( 'columns', array(
				...
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					1  => esc_html__('I Column', 'glidex-pro'),
					2  => esc_html__('II Columns', 'glidex-pro'),
					3  => esc_html__('III Columns', 'glidex-pro')
				),
				'description' => esc_html__( 'Number of columns you like to display your taxonomies.', 'glidex-pro' ),
				'default'      => 3,
			) );
			...
			$this->add_control( 'include', array(
				...
				'description' => esc_html__( 'Category ids separated by comma.', 'glidex-pro' ),
				'default' => ''
			) );

			$this->add_control( 'show_starting_price', array(
				'label'       => esc_html__( 'Show Starting Price', 'glidex-pro' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Choose true if you like to show starting price.', 'glidex-pro' ),
				'options'     => array(
			...
			$this->add_control( 'show_button', array(
				'label'       => esc_html__( 'Show Button', 'glidex-pro' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Choose true if you like to show button.', 'glidex-pro' ),
				'options'     => array(
					'false' => esc_html__('False', 'glidex-pro'),
					'true'  => esc_html__('True', 'glidex-pro'),
				),
		...
		$output = '';
			...
			if( is_array($categories) && !empty($categories) ) {
				...
				foreach( $categories as $category ) {
					...
					if($glidex_shop_cat_color != '') {
						$color_html .= '<div class="wdt-shop-category-listing-color" style="background-color:'.$glidex_shop_cat_color.';"></div>';
					}
							...
							$output .= '<div class="wdt-shop-category-listing-inner">';
								if($cat_image) {
									$output .= '<div class="wdt-shop-category-listing-image">';
										$output .= '<a href="'.get_term_link($category->term_id).'"><img src="'.esc_url($cat_image).'" alt="'.esc_html__('Category Image', 'glidex-pro').'" title="'.esc_html__('Category Image', 'glidex-pro').'" /></a>';
									$output .= '</div>';
								}
								$output .= '<div class="wdt-shop-category-meta-data">';
									...
									$output .= '<div class="wdt-shop-category-total-items">('.sprintf( esc_html__('%1$s Items', 'glidex-pro'), '<span>'.$category->count.'</span>' ).')</div>';
									if($settings['show_button'] == 'true') {
										$output .= '<a href="'.get_term_link($category->term_id).'" class="wdt-shop-cat-button button">'.esc_html__('View Details', 'glidex-pro').'</a>';
									}
								$output .= '</div>';
							$output .= '</div>';
						$output .= '</div>';
								...
								$output .= '<div class="wdt-shop-category-meta-data">';
									...
									if($settings['show_starting_price'] == 'true') {
										...
										if($wc_product_object) {
											...
											if($woo_price_html != '') {
												$output .= '<div class="wdt-shop-category-starting-price-html">'.sprintf(esc_html__('Starts from %1$s', 'glidex-pro'), $woo_price_html).'</div>';
											}
										}
