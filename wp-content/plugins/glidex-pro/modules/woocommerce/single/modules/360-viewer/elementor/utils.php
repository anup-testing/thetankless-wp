...
if(!function_exists('glidex_shop_product_images_360viewer_html')) {
	function glidex_shop_product_images_360viewer_html($attrs, $content = null) {
		...
		wp_enqueue_style( 'wdt-shop-product-single-images-360-viewer', glidex_shop_single_module_360_viewer()->module_dir_url() . 'elementor/widgets/assets/css/style'.$suffix.'.css', array (), false, 'all' );

		wp_enqueue_script('jquery-360viewer', glidex_shop_single_module_360_viewer()->module_dir_url() . 'elementor/widgets/assets/js/360-viewer'.$suffix.'.js', array ('jquery'), false, true);
		wp_enqueue_script('wdt-shop-product-single-images-360-viewer', glidex_shop_single_module_360_viewer()->module_dir_url() . 'elementor/widgets/assets/js/script'.$suffix.'.js', array ('jquery'), false, true);

		$output = glidex_shop_product_images_360viewer_render_html($attrs);

		return $output;

	}
	add_shortcode( 'glidex_shop_product_images_360viewer', 'glidex_shop_product_images_360viewer_html' );
}

if(!function_exists('glidex_shop_product_images_360viewer_render_html')) {
	function glidex_shop_product_images_360viewer_render_html($settings) {
					...
					$output .= '<div class="wdt-product-image-360-viewer-holder wdt-product-image-360-popup-viewer-holder '.$settings['class'].'">';
						...
						$output .= '<div class="wdt-product-image-360-viewer-container">';
							...
							$output .= '<div class="wdt-product-image-360-viewer" data-count="'.count($viewer360_gallery_ids).'">';
					   		...
					   		$output .= '<div class="wdt-product-image-360-viewer-close">'.esc_html__( 'Close', 'glidex-pro' ).'</div>';
