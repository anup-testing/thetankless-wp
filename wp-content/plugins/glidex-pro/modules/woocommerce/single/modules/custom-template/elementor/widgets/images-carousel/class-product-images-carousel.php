...
class Glidex_Shop_Widget_Product_Images_Carousel extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Product Single - Images Carousel', 'glidex-pro' );
	}
	...
	public function product_section() {
		...
		$this->start_controls_section( 'product_images_carousel_section', array(
			...
			$this->add_control( 'product_id', array(
				...
				'description' => esc_html__('Provide product id for which you have to display product iamges carousel. No need to provide ID if it is used in Product single page.', 'glidex-pro'),
			) );

			$this->add_control( 'include_featured_image', array(
				'label'        => esc_html__( 'Include Feature Image', 'glidex-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('If you wish, you can include featured image in this gallery.', 'glidex-pro'),
			...
			$this->add_control( 'include_product_labels', array(
				'label'        => esc_html__( 'Include Product Labels', 'glidex-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('If you wish, you can include product labels in this gallery.', 'glidex-pro'),
				'label_on'     => esc_html__( 'yes', 'glidex-pro' ),
				'label_off'    => esc_html__( 'no', 'glidex-pro' ),
				'default'      => '',
				'return_value' => 'true',
			...
			$this->add_control( 'enable_thumb_enlarger', array(
				'label'        => esc_html__( 'Enable Thumb Enlarger', 'glidex-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('If you wish, you can enable thumbnail enlarger in this gallery.', 'glidex-pro'),
				'label_on'     => esc_html__( 'yes', 'glidex-pro' ),
				'label_off'    => esc_html__( 'no', 'glidex-pro' ),
				'default'      => '',
				'return_value' => 'true',
	...
	public function carousel_section() {
		...
		$this->start_controls_section( 'product_carousel_section', array(
			...
			$this->add_responsive_control( 'carousel_slidesperview', array(
				'label'       => esc_html__( 'Slides Per View', 'glidex-pro' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Number slides of to show in view port.', 'glidex-pro' ),
			...
			$this->add_control( 'carousel_verticaldirection', array(
				'label'        => esc_html__( 'Enable Vertical Direction', 'glidex-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('To make your slides to navigate vertically.', 'glidex-pro'),
				'label_on'     => esc_html__( 'yes', 'glidex-pro' ),
				'label_off'    => esc_html__( 'no', 'glidex-pro' ),
				'default'      => '',
				'return_value' => 'true',
			...
			$this->add_control( 'carousel_thumbnailpagination', array(
				'label'        => esc_html__( 'Enable Thumbnail Pagination', 'glidex-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => esc_html__('To enable thumbnail pagination.', 'glidex-pro'),
				'label_on'     => esc_html__( 'yes', 'glidex-pro' ),
				'label_off'    => esc_html__( 'no', 'glidex-pro' ),
				'default'      => '',
				'return_value' => 'true',
			...
			$this->add_control( 'carousel_thumbnail_position', array(
				'label'       => esc_html__( 'Thumbnail Position', 'glidex-pro' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Number slides of to show in view port.', 'glidex-pro' ),
				'options'     => array(
					''      => esc_html__('Bottom', 'glidex-pro'),
					'left'  => esc_html__('Left', 'glidex-pro'),
					'right' => esc_html__('Right', 'glidex-pro'),
				),
			...
			$this->add_control( 'carousel_slidesperview_thumbnail', array(
				'label'       => esc_html__( 'Number Of Images - Thumbnail', 'glidex-pro' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Number of images to show in thumbnails.', 'glidex-pro' ),
				'options'     => array( 1 => 1, 2 => 2, 3 => 3, 4 => 4 ),
				'condition'   => array( 'carousel_thumbnailpagination' => 'true' ),
				'default'     => '4',
	        ) );
			...
			$output .= '<div class="wdt-product-image-gallery-holder '.$settings['class'].' '.$gallery_holder_class.' '.$gallery_holder_thumb_class.'">';
				...
				$output .= '<div class="wdt-product-image-gallery-container wdt-product-image-gallery-'.$settings['module_id'].' swiper-container" '.$media_carousel_attributes_string.'>';
			    	...
			    	if($settings['include_product_labels'] == 'true') {
						...
						ob_start();
						glidex_shop_woo_show_product_additional_labels($product);
					...
					$output .= '<div class="wdt-product-image-gallery-pagination-holder">';
						...
						if($settings['carousel_arrowpagination'] == 'true') {
							$output .= '<div class="wdt-product-image-gallery-arrow-pagination '.$settings['carousel_arrowpagination_type'].'">';
								$output .= '<a href="#" class="wdt-product-image-gallery-arrow-prev">'.esc_html__('Prev', 'glidex-pro').'</a>';
								$output .= '<a href="#" class="wdt-product-image-gallery-arrow-next">'.esc_html__('Next', 'glidex-pro').'</a>';
							$output .= '</div>';
						}
					...
					$output .= '<div class="wdt-product-image-gallery-thumb-container swiper-container">';
					    $output .= '<div class="wdt-product-image-gallery-thumb swiper-wrapper">';
		    				...
		    				if($settings['include_featured_image'] == 'true') {
								...
								$output .= '<div class="swiper-slide"><img src="'.esc_url($image_details[0]).'" title="'.esc_html__('Gallery Thumb', 'glidex-pro').'" alt="'.esc_html__('Gallery Thumb', 'glidex-pro').'" /></div>';
							}
		                    ...
		                    if(is_array($attachment_ids) && !empty($attachment_ids)) {
		                        ...
		                        foreach($attachment_ids as $attachment_id) {
	                                $image_details = wp_get_attachment_image_src($attachment_id, 'woocommerce_single');
	                               	$output .= '<div class="swiper-slide"><img src="'.esc_url($image_details[0]).'" alt="'.esc_html__('Gallery Thumb', 'glidex-pro').'" /></div>';
	                                $i++;
		                        }
		                    }
