...
if(!function_exists('glidex_shop_product_additional_tabs_exploded_html')) {
	function glidex_shop_product_additional_tabs_exploded_html($attrs, $content = null) {
		...
		$out = glidex_shop_product_additional_tabs_exploded_render_html($attrs);

		return $out;

	}
	add_shortcode( 'glidex_shop_product_additional_tabs_exploded', 'glidex_shop_product_additional_tabs_exploded_html' );
}


if(!function_exists('glidex_shop_product_additional_tabs_exploded_render_html')) {
	function glidex_shop_product_additional_tabs_exploded_render_html($settings) {
			...
			$output .= '<div class="wdt-product-tabs wdt-product-tabs-exploded '.$class.' '.$hide_title_class.' '.$scroll_class.'" '.$scroll_height_style_attr.'>';
				...
				if($settings['tab'] == 'custom_tab_1' || $settings['tab'] == 'custom_tab_2' || $settings['tab'] == 'custom_tab_3' || $settings['tab'] == 'custom_tab_4' || $settings['tab'] == 'custom_tab_5') {
					...
					if($settings['tab'] == 'custom_tab_1' && isset($product_additional_tabs[1])) {
						...
						$tab_key = 'glidex_'.strtolower(str_replace(' ', '', $tab_title));
						glidex_shop_woo_additional_product_tabs_content( $tab_key );
						$output .= ob_get_clean();
