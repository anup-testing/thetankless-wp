...
if( ! function_exists( 'glidex_shop_woo_additional_product_tabs' ) ) {

	function glidex_shop_woo_additional_product_tabs( $tabs ) {
		...
		if( is_array( $product_additional_tabs ) && !empty( $product_additional_tabs ) ) {
			...
			foreach( $product_additional_tabs as $product_additional_tab_key => $product_additional_tab ) {
				...
				$tab_key = 'glidex_'.strtolower($tab_key);

				$tabs[$tab_key] = array(
					'title' 	=> $tab_title,
					'priority' 	=> $priority,
					'callback' 	=> 'glidex_shop_woo_additional_product_tabs_content'
				);
	...
	function glidex_shop_woo_additional_product_tabs_content( $key ) {
	...
	add_filter( 'woocommerce_product_tabs', 'glidex_shop_woo_additional_product_tabs', 10 );
