...
if(!function_exists('glidex_shop_product_listing_html')) {
	function glidex_shop_product_listing_html($attrs, $content = null) {
		...
		$out = glidex_shop_products_render_html($attrs);

		return $out;

	}
	add_shortcode( 'glidex_shop_product_listing', 'glidex_shop_product_listing_html' );
}

if(!function_exists('glidex_shop_products_render_html')) {
	function glidex_shop_products_render_html($settings) {
		...
		if ( isset( $_REQUEST['glidex_shop_search_submit'] ) && !empty( $_REQUEST['glidex_shop_search_submit'] ) ) {
			$settings['page_requests'] = $_REQUEST;
		}

		$ps_nonce_verified = false;
		if ( isset( $settings['page_requests']['glidex_shop_product_search_nonce'] ) && wp_verify_nonce( $settings['page_requests']['glidex_shop_product_search_nonce'], 'glidex_shop_product_search' ) ) {
		...
		if($settings['enable_carousel'] == 'true') {
			...
			array_push($media_carousel_attributes, 'data-autoplayspeed="'.$settings['autoplay_speed'].'"');
			array_push($media_carousel_attributes, 'data-enableautoplay="'.$settings['enable_autoplay'].'"');
		...
		$type_class_instance = 'glidex_woo_listing_type_custom';
		if ( function_exists( $type_class_instance ) ) {

			if( $settings['product_style_template'] == -1 ) {
				$type_class_instance()->custom_template = glidex_customizer_settings('wdt-woo-shop-page-product-style-template' );
			} else {
		...
		$product_ul_class = $product_ul_class . $settings['columns'];
			...
			$output .= '<ul class="products '.$wrapper_class.' '.$classes.' '.esc_attr($product_ul_class).' ">';
				...
				if($settings['enable_carousel'] != 'true') {
					$output .= '<li class="product isotope-grid-sizer"><div class="'.glidex_woo_loop_column_class($settings['columns']).'"></div></li>';
				}
				...
				ob_start();
					...
					if($ps_nonce_verified) {
						if(isset($settings['page_requests']['glidex_shop_search_keyword']) && $settings['page_requests']['glidex_shop_search_keyword'] != '') {
							$args['s'] = $settings['page_requests']['glidex_shop_search_keyword'];
						}
					}
					...
					if($ps_nonce_verified) {
						...
						if(isset($settings['page_requests']['glidex_shop_search_categories']) && !empty($settings['page_requests']['glidex_shop_search_categories'])) {
							$glidex_shop_search_categories = $settings['page_requests']['glidex_shop_search_categories'];
						}

						if(!empty($glidex_shop_search_categories)) {
							$args['tax_query'][] = array (
														'taxonomy' => 'product_cat',
														'field'    => 'id',
														'terms'    => $glidex_shop_search_categories,
														'operator' => 'IN'
													);
						}
			...
			if($settings['show_pagination'] == 'true') {
				...
				$output .= glidex_shop_products_ajax_pagination($max_num_pages, $settings['current_page'], $settings['post_per_page'], 'glidex_shop_products_ajax_call', 'wdt-products-container', $shortcode_settings);
			}
			...
			if($settings['enable_carousel'] == 'true') {
				...
				$output .= '<div class="wdt-products-pagination-holder">';
					...
					if($settings['carousel_arrowpagination'] == 'true') {
						...
						$output .= '<a href="#" class="wdt-products-arrow-prev wdt-products-arrow-prev-'.esc_attr($settings['module_id']).'">'.esc_html__('Prev', 'glidex-pro').'</a>';
						$output .= '<a href="#" class="wdt-products-arrow-next wdt-products-arrow-next-'.esc_attr($settings['module_id']).'">'.esc_html__('Next', 'glidex-pro').'</a>';
						$output .= '</div>';
					}
...
if ( ! function_exists( 'glidex_shop_products_ajax_call' ) ) {

	function glidex_shop_products_ajax_call() {
		...
		if(isset($productpagination_nonce) && wp_verify_nonce($productpagination_nonce, 'productpagination_nonce')) {
			...
			$output = glidex_shop_products_render_html($shortcodeattrs_settings);
	...
	add_action( 'wp_ajax_glidex_shop_products_ajax_call', 'glidex_shop_products_ajax_call' );
	add_action( 'wp_ajax_nopriv_glidex_shop_products_ajax_call', 'glidex_shop_products_ajax_call' );

}

// Product Shortcode - Ajax Pagination

if ( ! function_exists( 'glidex_shop_products_ajax_pagination' ) ) {

	function glidex_shop_products_ajax_pagination($max_num_pages, $current_page, $post_per_page, $function_call, $output_div, $shortcode_attrs) {
		...
		if($max_num_pages > 1) {
			...
			$output .= '<div class="wdt-product-pagination wdt-product-ajax-pagination" data-postperpage="'.$post_per_page.'" data-functioncall="'.$function_call.'" data-outputdiv="'.$output_div.'"  data-shortcodeattrs="'.esc_js($shortcode_attrs).'" data-productpagination-nonce="'.wp_create_nonce('productpagination_nonce').'">';
				...
				if($current_page > 1) {
					$output .= '<div class="prev-post"><a href="#" data-currentpage="'.$current_page.'"><span class="prev">prev</span></a></div>';
				}
				...
				if ($current_page < $max_num_pages) {
					$output .= '<div class="next-post"><a href="#" data-currentpage="'.$current_page.'"><span class="next">Next</span></a></div>';
				}
...
if( ! function_exists( 'glidex_shop_single_product_nav' ) ) {
	function glidex_shop_single_product_nav() {
		...
		$output .= '<div class="wdt-single-product-nav">';
			...
			if ( ! empty( $prev ) ) {
				...
				$output .= '<div class="wdt-single-product-nav-btn product-nav-prev">';
					$output .= '<a href="'.esc_url( $prev->get_permalink() ).'">'.esc_html__('Previous product', 'glidex-pro').'<span class="product-nav-btn-icon"></span></a>';
			...
			$output .= '<a href="'.esc_url($shop_page_url).'" class="wdt-single-product-nav-back-btn"><span>'.esc_html__('Back to products', 'glidex-pro').'</span></a>';

			if ( ! empty( $next ) ) {

				$output .= '<div class="wdt-single-product-nav-btn product-nav-next">';
					$output .= '<a href="'.esc_url( $next->get_permalink() ).'">'.esc_html__('Next product', 'glidex-pro').'<span class="product-nav-btn-icon"></span></a>';
					$output .= '<div class="wdt-single-product-nav-intro-wrapper">';
						$output .= '<div class="product-nav-intro">';
							$output .= '<div class="product-nav-intro-image">';
