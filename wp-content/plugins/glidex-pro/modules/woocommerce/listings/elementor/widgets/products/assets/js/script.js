( function( $ ) {
	...
	var wdtShopProducts = function($scope, $){
		...
		const $autoplay_speed=$carouselItem.data('autoplayspeed');
		const $enableautoplay = $carouselItem.data('enableautoplay');
		const $autoheight = $carouselItem.data('autoheight');
		const $moduleId = $carouselItem.data('id');

		if($carouselresponsive === undefined) {
			return;
		}
		if($autoplay_speed === undefined) return;
		...
		if ($enableautoplay != '' && ($enableautoplay === true || $enableautoplay === 'true' || is_numeric($enableautoplay))) {
			wooswiperOptions.autoplay = {
				delay: $autoplay_speed, // Set autoplay speed here in milliseconds
		...
		if($scope.hasClass('elementor-element-edit-mode')) {
			...
			var customCSS = jQuery('body', parent.document).find('style[id="glidex-woo-non-archive-inline-css"]').text();
			jQuery('body', parent.document).find('style[id="glidex-woo-non-archive-inline-css"]').remove();
			...
			var customJS = jQuery('body', parent.document).find('script[id="glidex-woo-non-archive-js-after"]').text();
			jQuery('body', parent.document).find('script[id="glidex-woo-non-archive-js-after"]').remove();
