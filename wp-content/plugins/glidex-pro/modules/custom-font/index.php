...
if( !class_exists( 'GlidexProCustomFont' ) ) {
    class GlidexProCustomFont {
        ...
        function load_modules() {
            include_once GLIDEX_PRO_DIR_PATH.'modules/custom-font/taxonomy/index.php';
        }

        function load_customizer(){
            add_filter( 'glidex_customizer_custom_fonts', array( $this, 'fonts_list' ), 20 );
        }
        ...
        function elementor_group( $font_groups ) {
            ...
            if( count( $fonts ) > 0 ) {
                $new_group[ 'glidex_custom_fonts' ] = esc_html__( 'Custom Font', 'glidex-pro' );
        ...
        function add_elementor_fonts( $fonts ) {
            ...
            if( count( $fonts ) > 0 ) {
                foreach( $fonts as $key => $name ) {
                    $fonts[$name]   = 'glidex_custom_fonts';
                }
            }
        ...
        function load_frontend() {
            add_filter( 'glidex_add_inline_style', array( $this, 'custom_font_style' ) );
        }
        ...
        function get_fonts() {
            ...
            $fonts = array();
            $terms = get_terms( 'glidex_custom_fonts', array( 'hide_empty' => false ) );
        ...
        function get_font_links( $id ) {
            $meta = get_term_meta( $id, '_glidex_custom_font_options', true );
